...
def get_latest_CATool():
    global VERSION
    url = 'https://github.com/Minecraft-3DS-Community/CombinedAudioTool/releases'
    latest_releaseVersion = get_latest_release_number()
    print(f"Script Version: '{VERSION}'.\nLatest Release Version: '{latest_releaseVersion}'.\n")
    if VERSION == latest_releaseVersion:
        print(f"Your '{os.path.basename(__file__)}' Script Version is the Latest Release.")
        sys.exit(1)
    elif VERSION > latest_releaseVersion:
        print(f"Woah!!! Your script must be apart of the Development branch.")
        sys.exit(1)
    else:
        pass
    ...
    release_pattern = re.compile(r'/Minecraft-3DS-Community/CombinedAudioTool/releases/download/.+?/CATool.py')
    match = release_pattern.search(html)
    if not match:
        raise ValueError("Couldn't find the latest CATool.py link on the releases page")

    latest_release_link = f"https://github.com{match.group(0)}"
    with urllib.request.urlopen(latest_release_link) as response:
        catool_content = response.read()

    with open('CATool.py', 'wb') as f:
        f.write(catool_content)
